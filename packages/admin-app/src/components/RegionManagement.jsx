...
const fetchCountries = async () => {
  try {
    ...
    const q = query(countriesRef, where('status', '==', 'active'));
    const snapshot = await getDocs(q);
...
const toggleCountrySelection = (countryCode) => {
  setFormData(prev => {
    ...
    // Limit to reasonable number if needed, e.g., 20
    if (!isSelected && currentCountries.length >= 20) {
      toast.error('You can select up to 20 popular countries');
      return prev;
    }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {regions.map((region) => (
            <tr key={region.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                {region.popularCountries?.length || 0} countries
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {region.imageUrl ? (
                  <img
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-6">
            ...
            {/* Popular Countries Selection */}
            <div className="space-y-4">
              <h4 className="font-semibold text-gray-900 flex items-center gap-2">
                <Globe className="w-4 h-4" />
                Popular Countries ({formData.popularCountries?.length || 0} selected)
              </h4>
              <p className="text-xs text-gray-500">
                Calculated automatically by default. Select specific countries here to override and show only these.
              </p>
