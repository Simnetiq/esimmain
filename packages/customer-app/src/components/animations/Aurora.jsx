...
export default function Aurora({
  colorStops = ['#4975D4', '#6B9AE8', '#4975D4'],
  amplitude = 1.0,
  blend = 0.5,
  speed = 1.0
}) {
  ...
  useEffect(() => {
    ...
    import('ogl').then((OGL) => {
      ...
      // Use props from ref for initial values
      const initialProps = propsRef.current;
      const colorStopsArray = initialProps.colorStops.map(hex => {
      ...
      ctn._auroraResizeHandler = resize;
      ctn._auroraGl = gl;
    }).catch(err => {
      console.warn('Failed to load Aurora animation:', err);
    });

    return () => {
      cleanedUp = true;
      cancelAnimationFrame(animateId);
      if (ctn && ctn._auroraResizeHandler) {
        window.removeEventListener('resize', ctn._auroraResizeHandler);
      }
      if (ctn && ctn._auroraGl) {
        const gl = ctn._auroraGl;
